...
func (basic *Basic) GetAccountBasicInfo(ctx context.Context) (*AccountBasicInfo, error) {
	ak, err := basic.Context.GetAuthAccessToken(ctx, basic.appID)
	if err != nil {
		return nil, err
	}
...
func (basic *Basic) CheckNickName(ctx context.Context, nickname string) (*CheckNickNameResp, error) {
	ak, err := basic.Context.GetAuthAccessToken(ctx, basic.appID)
	if err != nil {
		return nil, err
	}
...
// SetNickName 设置小程序名称
func (basic *Basic) SetNickName(ctx context.Context, nickname string) (*SetNickNameResp, error) {
	return basic.SetNickNameFull(ctx, &SetNickNameParam{
		NickName: nickname,
	})
}
...
func (basic *Basic) SetNickNameFull(ctx context.Context, param *SetNickNameParam) (*SetNickNameResp, error) {
	ak, err := basic.Context.GetAuthAccessToken(ctx, basic.appID)
	if err != nil {
		return nil, err
	}
...
func (basic *Basic) SetSignature(ctx context.Context, signature string) error {
	ak, err := basic.Context.GetAuthAccessToken(ctx, basic.appID)
	if err != nil {
		return err
	}
...
func (basic *Basic) GetSearchStatus(ctx context.Context, signature string) (*GetSearchStatusResp, error) {
	ak, err := basic.Context.GetAuthAccessToken(ctx, basic.appID)
	if err != nil {
		return nil, err
	}
...
func (basic *Basic) SetSearchStatus(ctx context.Context, status int) error {
	ak, err := basic.Context.GetAuthAccessToken(ctx, basic.appID)
	if err != nil {
		return err
	}
	url := fmt.Sprintf("%s?access_token=%s", setSearchStatusURL, ak)
	data, err := util.PostJSONContext(ctx, url, map[string]int{
		"status": status,
	})
	if err != nil {
		return err
...
func (basic *Basic) SetHeadImage(ctx context.Context, imgMediaID string) error {
	return basic.SetHeadImageFull(ctx, &SetHeadImageParam{
...
func (basic *Basic) SetHeadImageFull(ctx context.Context, param *SetHeadImageParam) error {
	ak, err := basic.Context.GetAuthAccessToken(ctx, basic.appID)
	if err != nil {
		return err
	}
