...
func (c *Context) GetAuthAccessToken1(ctx context.Context, appId string) (string, error) {
	...
	if len(authorizerAccessToken.AuthorizerAccessToken.RefreshToken) == 0 {
		return "", fmt.Errorf("refresh token %s is null", appId)
	}
...
func (c *Context) refreshAuthToken(ctx context.Context, appId, refreshToken string) (string, error) {
	...
	if ret.ExpiresIn == 0 {
		return "", fmt.Errorf("refresh token url %s post json context error", appId)
	}
