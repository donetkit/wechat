...
func (s *VirtualPayment) QueryUserBalance(ctx context.Context, in *QueryUserBalanceRequest) (out QueryUserBalanceResponse, err error) {
	...
	if address, err = s.requestAddress(ctx, params); err != nil {
		return
	}
...
// requestURL .组合 URL
func (s *VirtualPayment) requestAddress(ctx context.Context, params URLParams) (url string, err error) {
	switch params.Path {
	...
	if params.AccessToken, err = s.ctx.GetAccessToken(ctx); err != nil {
		return
	}
