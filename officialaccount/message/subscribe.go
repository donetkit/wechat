...
type SubscribeeMessage struct {
	...
	Data        map[string]*SubscribeeDataItem `json:"data"`           // 必须, 模板数据
	MiniProgram struct {
...
// GetCategory 获取公众号类目
func (tpl *Subscribe) GetCategory(ctx context.Context) (categoryList []*PublicTemplateCategory, err error) {
	var accessToken string
	accessToken, err = tpl.GetAccessToken(ctx)
	if err != nil {
		return
...
func (tpl *Subscribe) GetPubTplKeyWordsByID(ctx context.Context, titleID string) (keyWordsList []*PublicTemplateKeyWords, err error) {
	var accessToken string
	accessToken, err = tpl.GetAccessToken(ctx)
	if err != nil {
		return
...
func (tpl *Subscribe) GetPublicTemplateTitleList(ctx context.Context, ids string, start int, limit int) (count int, templateTitleList []*PublicTemplateTitle, err error) {
	var accessToken string
	accessToken, err = tpl.GetAccessToken(ctx)
	if err != nil {
		return
