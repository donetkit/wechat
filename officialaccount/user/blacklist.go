...
func (user *User) GetAllBlackList(ctx context.Context) (openIDList []string, err error) {
	...
	for {
		// 获取列表（每次1k条）
		if userlist, err = user.GetBlackList(ctx, beginOpenid); err != nil {
			return nil, err
		}
...
func (user *User) BatchBlackList(ctx context.Context, openidList ...string) (err error) {
	return user.batch(ctx, batchblacklistURL, "BatchBlackList", openidList...)
}
...
func (user *User) BatchUnBlackList(ctx context.Context, openidList ...string) (err error) {
	return user.batch(ctx, batchunblacklistURL, "BatchUnBlackList", openidList...)
}

// batch 公共方法
func (user *User) batch(ctx context.Context, url, apiName string, openidList ...string) (err error) {
	// 检查参数
